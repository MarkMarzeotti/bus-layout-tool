  ...
  <div id="area" class="w-8/12 h-full flex items-center justify-center overflow-scroll relative">
    <div v-if="$store.state.editingSave !== false">
      <div class="z-50 absolute top-0 right-0 mt-4 mr-4 border border-gray-300 rounded items-stretch flex">
        ...
        <span class="bg-white text-sm text-gray-600 px-3 w-20 flex justify-center items-center">{{ $store.state.saves[$store.state.editingSave].workspace.scale * 100 }}%</span>
      ...
      <div id="workspace" class="bg-gray-100 border-4 border-gray-400 relative box-content" :style="workspaceStyle">
  ...
  methods: {
    zoomOut: function () {
      if (this.$store.state.editingSave !== false) {
        this.saves[this.editingSave].workspace.scale = this.$store.state.saves[this.$store.state.editingSave].workspace.scale - .125;
